"""Build reel 1 (Day 6) overlays — PIL PNG frames, encode later to ProRes 4444.
...
Slots (all 1080x1920 RGBA, 30 fps, top-band rule: everything inside y 0-760):
  ...
  slot_gfx_tech1  9.0s  PEAD card — flat price, earnings-beat marker, drift line
  slot_gfx_tech2 16.5s  SEC filings card — doc stack, FREE/PUBLIC badges, $0 pill
  slot_gfx_reject 10.2s rejected-techniques card — two X rows + SKIPPED stamp
  slot_gfx_steps 10.9s  roadmap card — 3 step rows + KTrade v1 banner + check

Payoff sync targets (output timeline, from edl_v1.json offsets):
  day6:  "six" lands out 1.36  -> pop lands t=1.30 (overlay starts 0.0)
  tech1: "PEAD" out 25.34      -> title lands t=0.44 (starts 24.90)
         "beating expectations" out 30.8-32.1 -> marker t=5.9, drift t=6.1-8.1
  tech2: "SECs" out 41.19      -> title lands t=0.39 (starts 40.80)
         "free" out 48.09 t=7.29 / "public" out 48.55 t=7.75
         "without spending" out 56.15 t=15.35 ($0 pill)
  reject:"money" out 63.44 t=4.94 / "corporations" out 65.68 t=7.18
         "terrible idea" out 67.14 t=8.64 (stamp)
  steps: "step two" out 69.55 t=0.60 / "step three" out 72.99 t=4.04
         "KTrade" out 77.03 t=8.08 / "done" out 79.25 t=10.30
"""
...
def render_day6(im, t):
    ...
    # corner drafting brackets draw in 0.3-0.8s
    bp = ease_out_cubic(prog(t, 0.3, 0.5))
    if bp > 0:
        L = int(46 * bp)
        wdt = 5
    ...
    # "DAY 6" pops with overshoot, landing at t=1.30 (0.9-1.3)
    pp = prog(t, 0.9, 0.4)
    if pp > 0:
        s = ease_out_back(pp)
    ...
    # subtitle fades in 1.6-2.0s, gentle float afterwards
    sp = ease_out_cubic(prog(t, 1.6, 0.4))
    if sp > 0:
        d2 = ImageDraw.Draw(layer)
        bob = int(2 * math.sin((t - 1.6) * 2.2)) if t > 2.0 else 0
        fnt = f_b(40)
...
def render_tech1(im, t):
    ...
    # header lands with spoken "PEAD" (t=0.44)
    hp = ease_out_cubic(prog(t, 0.25, 0.35))
    if hp > 0:
        a = int(255 * hp)
    ...
    # earnings-beat marker pops at t=5.9 (synced to "beating ... expectations")
    mp = prog(t, 5.9, 0.35)
    if mp > 0:
        s = ease_out_back(mp)
        r = int(13 * s)
    ...
    # drift line rises 6.1-8.1s
    dp = ease_in_out_cubic(prog(t, 6.1, 2.0))
    if dp > 0:
        n = max(2, int(40 * dp))
        pts = []
        for i in range(n):
    ...
    # drift label fades 7.5-8.0s
    lp = ease_out_cubic(prog(t, 7.5, 0.5))
    if lp > 0:
        a = int(255 * lp)
        d.text((mark_x + 130, base_y - 120), "DRIFT (WEEKS)", font=f_b(28),
               fill=(GREEN[0], GREEN[1], GREEN[2], a))
...
def render_tech2(im, t):
    ...
    badge("FREE", CX0 + 60, 7.29, GREEN)
    badge("PUBLIC", CX0 + 290, 7.75, GREEN)

    # $0 pill pops at "without spending (a dime)" t=15.35
    p = prog(t, 15.35, 0.35)
    if p > 0:
        s = ease_out_back(p)
        bw, bh = 740, 70
        cxb, cyb = (CX0 + CX1) // 2, y0 + 330
        w2, h2 = int(bw * s) // 2, int(bh * s) // 2
        rounded(d, (cxb - w2, cyb - h2, cxb + w2, cyb + h2), 34,
                GOLD + (int(255 * min(1.0, p * 2)),))
        if p > 0.5:
            fnt = f_b(34)
            msg = "$0 — TRACK EVERY CORPORATION"
            tw = d.textlength(msg, font=fnt)
            d.text((cxb - tw / 2, cyb - 24), msg, font=fnt, fill=(40, 30, 6, 255))
...
def render_reject(im, t):
    ...
    # two rejection rows; X pops sync to "money" (t=4.94) / "corporations" (t=7.18)
    rows = [
        ("NEED TOO MUCH MONEY", 1.0, 4.94),
        ("COMPETE WITH WALL ST GIANTS", 1.4, 7.18),
    ]
    BH, GAP = 78, 22
    top = y0 + 110
    ...
    # diagonal SKIPPED stamp across the whole card at "terrible idea" (t=8.64)
    sp = prog(t, 8.64, 0.35)
    if sp > 0:
...
def render_steps(im, t):
    ...
    top = y0 + 82
    rows = [
        ("1. RESEARCH", 0.40, True),    # already done in prior videos
        ("2. CODE THE TECHNIQUES", 0.60, False),   # "step two" t=0.60
        ("3. BACKTEST + PAPER TRADE", 4.04, False),  # "step three" t=4.04
    ]
    ...
    # gold v1 banner pops at "KTrade" (t=8.08)
    bp = prog(t, 8.08, 0.35)
    if bp > 0:
        s = ease_out_back(bp)
        bw, bh = 560, 62
    ...
    # green check next to banner at "done" (t=10.30)
    cp = prog(t, 10.30, 0.3)
    if cp > 0:
        s = ease_out_back(cp)
...
SLOTS = {
    ...
    "slot_gfx_tech1":  (9.0,  render_tech1),
    "slot_gfx_tech2":  (16.5, render_tech2),
    "slot_gfx_reject": (10.2, render_reject),
    "slot_gfx_steps":  (10.9, render_steps),
}
