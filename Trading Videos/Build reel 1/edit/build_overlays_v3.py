"""Build reel 1 (Day 6) overlays v3 — PIL PNG frames, encode later to ProRes 4444.
...
def render_tech1(im, t):
    ...
    # drift label fades 11.9-12.4s
    lp = ease_out_cubic(prog(t, 11.9, 0.5))
    if lp > 0:
        a = int(255 * lp)
...
def render_hook(im, t):
    """Cold-open gold pill, no card — pops on "track" (t=0.28), holds to the cut."""
    layer = Image.new("RGBA", (W, H), (0, 0, 0, 0))
    d = ImageDraw.Draw(layer)
    p = prog(t, 0.03, 0.35)
    if p > 0:
